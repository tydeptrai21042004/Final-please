...
def evaluate(
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if measure_latency and device.type == "cuda" and torch.cuda.is_available():
            torch.cuda.synchronize(device)
        ...
        if measure_latency and device.type == "cuda" and torch.cuda.is_available():
            torch.cuda.synchronize(device)
        if measure_latency:
            elapsed_forward += time.perf_counter() - start
        # Criterion evaluation is intentionally outside the timed model-forward
        # region.  The previous implementation reported inference + loss time,
        # which is not deployment latency and can vary by task type.
        with _amp_context(device, use_amp):
